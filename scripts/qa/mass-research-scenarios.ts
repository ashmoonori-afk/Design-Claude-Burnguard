...
async function migrate(db: Database): Promise<void> { const run = await loadProductFunction<(db: Database, directory: string) => Promise<void>>("db/migrate.ts", "runMigrationsFrom"); await run(db, new URL("../../packages/backend/src/db/migrations", import.meta.url).pathname); }
