...
- Windows/macOS managed auto-update 채널 (Phase 5 P5.1)
- Windows SmartScreen / macOS notarization 서명 (P4.5)
...
- 아래 CLI 중 하나 이상이 `PATH`에 있어야 함
  - `claude`
  - `codex`
- Python 3.10+ (선택 — PDF / PPTX 디자인 시스템 업로드 기능에만 필요).
  의존성 목록은
  [`packages/backend/requirements.txt`](packages/backend/requirements.txt),
  또는 Settings → "Python for uploads" → Install pypdf 버튼으로 자동 설치
  가능합니다.
