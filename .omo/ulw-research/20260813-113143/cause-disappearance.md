...
| Proposed cause | Disappearance test | Result | Planning consequence |
|---|---|---|---|
| Missing conversation continuity is caused by one-shot CLI turns. | Resume one native session across a cold BurnGuard restart and compare non-file context retention. | Pending implementation fixture; installed Claude and Codex expose native resume paths. | Plan a bounded resume slice before a persistent process. |
| Users cannot safely understand rollback because snapshots have no visible diff. | Generate add/modify/delete changes, persist a bounded diff, then restore and confirm the diff closes. | Current snapshots provide the required before-state; viewer is absent. | Support a read-only per-turn diff without Git. |
