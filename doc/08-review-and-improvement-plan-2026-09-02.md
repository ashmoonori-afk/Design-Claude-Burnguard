...
이 문서는 구현 전에 작성되었으므로, 실행 과정에서 확인한 사실에 따라 다음이 달라졌다.

- **T3의 진단이 바뀌었다.** 계측 결과 `chromium.launch()`는 단순히 느린 것이 아니라
  Bun 이벤트 루프 전체를 멈춘다. 따라서 요청 단위 타임아웃만으로는 부족했고, 자식
  프로세스 실행 가능 여부 프로브(`chromium-capability.ts`)를 도입해 `launchChromium`
  진입 지점에서 차단하는 방식으로 구현했다. 네이티브 CDP 클라이언트(T3-b)는 여전히
  후속 과제이다.
- **썸네일이 앱 전체를 막고 있었다.** 홈 화면이 카드마다 보내는 썸네일 요청이 브라우저
  연결 한도를 채우고 서버까지 멈춰, 프로젝트를 아예 열 수 없었다. 계획에는 없던
  응답 기한을 썸네일 경로에 추가했다.
- **T4의 범위가 넓어졌다.** 테스트 파일뿐 아니라 `scripts/qa/mass-research-scenarios.ts`도
  같은 경로 버그를 갖고 있어 함께 고쳤다.
- **E2E는 `scripts/qa/e2e-smoke.mjs`로 저장소에 포함했다.** 계획대로 Node에서 실행하며,
  샌드박스 iframe 내부는 외부 드라이버가 조회할 수 없어 `srcdoc` 속성과 서버 상태로
  검증한다.
