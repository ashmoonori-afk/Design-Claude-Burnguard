...
| **P4.6** | 🔲 | **Easy install / launch packages.** Add user-friendly distributables so people can install and open BurnGuard without cloning the repo or touching a terminal. **Windows:** ship a real installer (`Setup.exe` or `.msi`) that places the app in a managed install root, creates shortcuts, and can run first-launch bootstrap checks. **macOS:** ship a user-friendly package (`.dmg` with `.app`, optionally later `.pkg`) with a clear drag-and-drop install path and first-run bootstrap inside the app. Bootstrap scope: verify or guide Chromium / Python prerequisites, create the managed app root, and hand off to the normal app launch. This is separate from auto-update: the goal here is frictionless first install and first launch. | `scripts/build-installer-win.ts` (new), `scripts/build-package-mac.ts` (new or extends `build-mac.ts`), first-run bootstrap hooks, release packaging workflow, docs / first-run copy | A non-technical user can download one package, install or open BurnGuard in one guided flow, and reach the app without manual repo, shell, or dependency setup |

| **P4.7** | 🟡 | **Sample library hardening + UX.** Bundles six sample-system improvements surfaced after the 2026-04-24 IRIS-pptx leak audit. **(a) ✅ Trademark-safe sample brands** — sample DS rebranded to fictional **Northvale Capital** (was Goldman Sachs) + **Splash** (was Toss) across `design system sample/` (README, SKILL, CSS, preview HTMLs, UI-kit JSX/HTML), all 12 GS logo files replaced with 6 generic SVG monogram placeholders, and the rename threaded through `design-systems-list.json`, `projects-list.json`, frontend mocks, backend bootstrap/seed, and docs. **(b) `uploads/` leak guard:** ✅ `.gitignore` rule + `.gitkeep` placeholder shipped (`95a4ace`); ✅ `seedSampleDesignSystem` now skips the `uploads/` subtree when copying the sample DS into `~/.burnguard/data/systems/` via a path-segment filter (this commit). Remaining: a CI / pre-commit guard that hard-fails when a binary lands under `design system sample/uploads/**` — defer until CI workflow exists. **(c) ✅ Examples-tab filter fix** — `listHomeProjects("examples")` previously filtered to `type === "from_template"` only, so the real working tutorial + prompt-sample projects (typed `prototype` / `slide_deck`) never surfaced there; the only thing the user saw was one placeholder template fixture. The filter is now extracted as a pure `isExampleProject(row)` helper that also admits any row whose name carries the seeded `[burnguard:tutorial]` or `[burnguard:prompt-sample]` prefix. Tutorials, the four prompt-samples, and the placeholder template all show in Examples; regular user projects (even ones that happen to mention the tag mid-sentence) do not. **(d) ✅ Restore-samples affordance** — new `POST /api/home/restore-samples` route in the home router calls `seedTutorialsOnce` (already idempotent: only the missing tagged projects are recreated). Examples tab in `HomeView` carries an explanatory line plus a Restore samples button that triggers a `useMutation`, invalidates the projects query on success, and shows a toast either way. Deleting any tutorial or prompt-sample is now a safe operation — one click brings the built-in set back. **(e) ✅ One-click "Try this prompt"** — each rendered prompt-sample now ships a styled `<form action="/api/home/use-sample/{slug}" method="POST" target="_top">` button beneath the source-prompt `<pre>`. The new `POST /api/home/use-sample/:slug` route looks the slug up via the new `getPromptSampleBySlug` helper, calls `createProjectRecord` to mint a fresh `prototype` project named `Try: <sample>`, and 302-redirects to `/projects/{id}?prefill_prompt={base64url}`. ProjectView reads the param on first mount, decodes via the URL-safe-base64 + atob path, hands it to ChatPane → Composer as `initialText`, and strips the param from the URL via `setSearchParams({}, { replace: true })` so a refresh doesn't re-prefill on top of whatever the user has typed. The composer sits ready with the prompt; user only has to click Send. **(f) ✅ Backfill placeholder fixtures** — chose backfill over retire (user direction). New `seeded-project-html.ts` ships a real, self-contained ~3-5 KB starter artifact for each of the five non-archived `homeProjectFixtures` rows in the appropriate genre and brand vocabulary: a Northvale-branded Series A investor landing, a 6-slide Northvale quarterly review deck (with the deck-stage runtime co-located), a Splash consumer-fintech template landing, a no-DS portfolio playground (palette + type stack + components sandbox), and a Salt-style market-update microsite. The seed loop now writes these under each project's `dirPath` on first run; archived rows skip the file write. Opening any Home card now lands on a non-empty canvas. **(g) 🟡 Sample variety** — prompt-sample axis ✅ shipped: two new entries cover the previously-missing axes — `daon-korean-saas` is a fully Korean-localized SaaS landing (Pretendard typography, ₩-priced tier table, Korean copy throughout the prompt) using the existing editorial layout, and `pulse-fund-ops-dashboard` is the first non-landing sample with its own dedicated `dashboard` layout that renders a real institutional admin/analytics console (sidebar nav, KPI tiles, holdings table, activity feed). Six prompt-samples ship now (was four). Remaining work: an additional working sample design system in a different vertical (consumer SaaS or editorial) — that is a ~30-file authoring task on the order of the original Northvale system, so it is queued as its own slice rather than bundled here. | `design system sample/` (rename + uploads gate), `.gitignore`, `packages/backend/src/bootstrap.ts` (uploads skip), `packages/backend/src/db/seed-tutorials.ts` (variety + restore export), `packages/backend/src/db/seed.ts` (Examples filter / `is_example` column), `packages/backend/src/fixtures/*.json` (rename + decide on placeholders), `packages/frontend/src/views/HomeView.tsx` (Restore-samples button + Examples filter wiring), prompt-sample HTML renderer in `seed-tutorials.ts` (Try-this-prompt button + new POST handler in `routes/session.ts`), CI workflow / pre-commit hook for the `uploads/` guard | (a) `git grep -i "goldman\|toss design\|salt design"` → empty across `design system sample/`, fixtures, and mocks. (b) Adding any binary under `design system sample/uploads/` is rejected before commit. (c) Examples tab shows the 6 real seeded examples (2 tutorials + 4 prompt-samples), not just one placeholder. (d) Deleting a tutorial then clicking "Restore samples" re-creates it without restarting the app. (e) Opening any prompt-sample → "Try this prompt" → a new session lands on the sample's DS with the prompt already submitted. (f) Every card visible on Home opens to a non-empty canvas. (g) At least one additional sample DS and one additional non-landing prompt-sample ship with the bundle. |
